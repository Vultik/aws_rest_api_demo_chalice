...
@app.authorizer()
def jwt_auth(auth_request: Any) -> Any:
    token = auth_request.token

    decoded = auth.decode_jwt_token(token)

    if not decoded:
        return AuthResponse(routes=[], principal_id=None)  # no access to anything

    return AuthResponse(routes=['*'], principal_id=decoded['sub'])



def get_users_db() -> Any:
    global _USER_DB
    if _USER_DB is None:
...
def get_authorized_username(current_request: Any) -> str:
